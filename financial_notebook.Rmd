...
Create the debt ratio in each dataset. Add leading zeros. Merge codes.
```{r}
#municipalities
...
t_watfin$FIPS_wc<- paste(t_watfin$FIPS_Code_State,t_watfin$FIPS_County,t_watfin$FIPS_Place, sep="")
t_watfin$FIPS<-NA
```

```{r}
n1<-merge(watfin,masterFIPS, by.x="FIPS",by.y="FIPS")
n2<-merge(t_watfin,masterFIPS, by.x="FIPS_wc",by.y="FIPS")
```
...
output_Mc<-match_df(watfin, masterFIPS, on = "FIPS_wc")
#subset the large financial database using the master list of FIPS codes
output_C <- match_df(c_watfin, masterFIPS, on = "FIPS")
output_Cc<-match_df(watfin, masterFIPS, on = "FIPS_wc")
#subset the large financial database using the master list of FIPS codes
output_T <- match_df(t_watfin, masterFIPS, on = "FIPS")
output_Tc<-match_df(watfin, masterFIPS, on = "FIPS_wc")
```
